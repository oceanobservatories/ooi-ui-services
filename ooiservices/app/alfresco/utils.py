#!/usr/bin/env python
'''

'''
__author__ = 'M@Campbell'

import os
import requests
import json
from cmislib.model import CmisClient

from flask import current_app


def make_alfresco_conn():
    '''
    Set up the config variables.
    '''
    ALFRESCO_URL = current_app.config['ALFRESCO_CMIS_URL'] or \
        os.environ.get('ALFRSCO_CMIS_URL')
    ALFRESCO_UN = current_app.config['ALFRESCO_USERNAME'] or \
        os.environ.get('ALFRESCO_USERNAME')
    ALFRESCO_PW = current_app.config['ALFRESCO_PASSWORD'] or \
        os.environ.get('ALFRESCO_PW')
    ALFRESCO_ID = current_app.config['ALFRESCO_REPO_ID'] or \
        os.environ.get('ALFRESCO_REPO_ID')

    # create the cmis client
    client = CmisClient(ALFRESCO_URL, ALFRESCO_UN, ALFRESCO_PW)

    # connect to the alfresco server and return the repo object
    repo = client.getRepository(ALFRESCO_ID)

    # this will be what we work with primarily
    # returns a repo object
    return repo


def make_alfresco_ticket():
    '''
    Setup the config variables.
    '''
    ALFRESCO_URL = current_app.config['ALFRESCO_TICKET_URL'] or \
        os.environ.get('ALFRSCO_TICKET_URL')
    ALFRESCO_UN = current_app.config['ALFRESCO_USERNAME'] or \
        os.environ.get('ALFRESCO_USERNAME')
    ALFRESCO_PW = current_app.config['ALFRESCO_PASSWORD'] or \
        os.environ.get('ALFRESCO_PW')

    # make a post request, which is the preferred method to get
    # a alfresco ticket. ticket will last 1 hour.
    response = requests.post(ALFRESCO_URL,
        headers={'Content-Type': 'application/json'},
        data=json.dumps({'username': ALFRESCO_UN, 'password': ALFRESCO_PW}))
    return response


def make_alfresco_query(query):
    '''
    query the alfresco server for all documents matching the search param.
    **CAUTION: THIS MAY TAKE A WHILE TO RESPOND**
    '''
    # use this files connection method
    repo = make_alfresco_conn()

    # issue the query
    results = repo.query("select * from cmis:document where contains('\"%s\"')"
        % query)

    return results


def make_alfresco_download_link(id, ticket):
    '''
    In order to download a document, a specific URL needs to be used.
    Each link needs to be passed in the new ticket in order to download.

    param: ticket ; this is the ticket that will be used to authenticate
        the request
    '''

    # then lets split out the object id and use use the hex portion . . .
    arrID = id.split('/')
    hex_id = arrID[3]

    # now we need the download root URI
    ALFRESCO_URL = current_app.config['ALFRESCO_DL_URL'] or \
        os.environ.get('ALFRESCO_DL_URL')

    # and before we return, we just need to combine all three parts
    # to make a nice url for a authenticated user to download.

    download_url = ''.join((ALFRESCO_URL, '/', hex_id, '?alf_ticket=', ticket))
    return download_url
