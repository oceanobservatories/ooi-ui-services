...
def _compile_asset_rds():
    ...
    try:
        ...
        if isinstance(toc, list):
            if debug: print '\n debug -- len(_toc): ', len(_toc)
            message = 'uframe toc returned is not properly structured (list, not dictionary); '
            message += 'unable to process for reference designators.'
            if debug: print '\n debug -- message: ', message
            raise Exception(message)

        if not toc:
            message = 'uframe toc is None or empty, unable to process for reference designators.'
            if debug: print '\n debug -- message: ', message
            raise Exception(message)
        ...
        if isinstance(toc, dict):
            ...
            message = 'uframe toc content is not a dictionary; failure to retrieve toc dictionary.'
            print '\n debug -- message: ', message
            raise Exception(message)
        ...
        if reference_designators and toc_only:
            if debug: print '\n reference_designators(%d): %s' % (len(reference_designators), reference_designators)
        ...
        if debug:
            print '\n debug -- compile_asset_rds - type(result): ', type(result)
            print '\n debug -- compile_asset_rds - result(%d): %s' % (len(result), json.dumps(result, indent=4, sort_keys=True) )

        return result, rds_wo_assets, reference_designators #list_of_ids
...
def process_toc_information_reference_designators(toc):
    ...
    try:
        ...
        Namely, the difference being the list of omitted_reference_designators shown above.
        """
        if debug:
...
def get_asset_id_by_reference_designator(rd, uframe_url=None, timeout=None, timeout_read=None):
    ...
    Modified to handle irregular reference designators - those which are syntactically correct excepting the sensor,
    which may be less than standard nine characters in length and have port values which are not 2 characters in length.
    """
    info = False
    debug = False
    ids = []
...
def _compile_asset_rds():
    ...
    try:
        ...
        if reference_designators and toc_only:
            ...
            """
            # Put instruments before nodes and nodes before moorings
            _reference_designators = []
            for rd in reference_designators:
                if len(rd) > 14:
                    if rd not in _reference_designators:
                        _reference_designators.append(rd)
        ...
        message = 'ConnectionError for _xcompile_asset_rds.'
        current_app.logger.info(message)
        return {}, []
    except Timeout:
        message = 'Timeout for _xcompile_asset_rds.'
        current_app.logger.info(message)
        return {}, []
    except Exception as err:
        message = err.message
...
def get_asset_id_by_reference_designator(rd, uframe_url=None, timeout=None, timeout_read=None):
    ...
    http://uframe-3-test.ooi.rutgers.edu:12587/xasset/cal?refdes=CP01CNSP-SP001-06-DOSTAJ000
    """
    info = False
    debug = False
    ids = []
    ...
    try:
        ...
        # Get reference designator components (mooring, platform, instrument) and form url
        len_rd = len(rd)
        url_root = "/".join([uframe_url, 'deployments/query?refdes='])  # 'asset/cal?refdes='
        #if debug: print '\n debug -- url_root: ', url_root
        if len_rd == 27 and ('-' in rd):
            mooring, platform, instrument = rd.split('-', 2)
        ...
        # Query uframe for reference designator asset ids.
        if debug: print '\n debug --- url: ', url
        response = requests.get(url, timeout=(timeout, 15)) #timeout_read)) # todo review
        if response.status_code != 200:
            if info:
                message = '(%d) Failed to get uframe asset id for reference designator: %s' % (response.status_code,rd)
                current_app.logger.info(message)
...
def get_instrument_deployments_list(rd):
    ...
    result = []
    try:
        if debug: print '\n debug -- get_instrument_deployments_list for rd: ', rd
        ...
        uframe_url, timeout, timeout_read = get_uframe_deployments_info()
        url = '/'.join([uframe_url, 'deployments', 'inv', query_rd])
        ...
        if debug: print '\n debug -- [get_instrument_deployments_list] (list) result: ', result
...
def get_instrument_deployments(rd):
    ...
    result = []
    try:
        if debug: print '\n debug -- get_instrument_deployments for rd: ', rd
...
def get_rd_deployments(rd):
    ...
    result = []
    try:
        if debug: print '\n debug -- get_rd_deployments for rd: ', rd
        ...
        # Get uframe deployments request variables
        uframe_url, timeout, timeout_read = get_uframe_deployments_info()
        url = '/'.join([uframe_url, 'deployments', 'query'])
        url += '?refdes=' + rd
        ...
        result = response.json()
        #if debug: print '\n debug -- [get_rd_deployments] (list) result: ', result
        return result

    except ConnectionError:
        message = 'ConnectionError for uframe /deployments/query?refdes=%s.' % rd
        current_app.logger.info(message)
        return []
    except Timeout:
        message = 'Timeout for for uframe /deployments/query?refdes=%s.' % rd
        current_app.logger.info(message)
        return []
    except Exception as err:
        message = str(err)
        print '\n debug -- exception [get_rd_deployments]: ', message
        current_app.logger.info(message)
        return []
...
def get_deployment_asset_ids(deployment):
    ...
    request:
        http://uframe-3-test.ooi.rutgers.edu:12587/deployments/inv/CE01ISSP/SP001/10-PARADJ000/1
...
def _get_rd_assets():
    """ Get cached 'rd_assets', if not available generate, cache and return 'rd_assets' dictionary.
    """
    debug = False
    rd_assets = {}
    CACHE_TIMEOUT = 86400       # todo - review this use of cache, etc.
    try:
...
def get_instrument_deployment_work(rd):
    """ For instrument rd create deployment information breakout. Return dict or empty dict. Log exceptions.
    """
    debug = False
    work ={}
    try:
...
def get_mooring_deployment_work(rd):
    """ For mooring rd create deployment information breakout. Return dict or empty dict. Log exceptions.
    """
    debug = False
    work ={}
    try:
...
def get_platform_deployment_work(rd):
    """ For platform rd create deployment information breakout. Return dict or empty dict. Log exceptions.
    """
    debug = False
    work ={}
    try:
