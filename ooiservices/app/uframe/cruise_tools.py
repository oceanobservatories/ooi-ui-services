...
# Get cruise inventory by array.
def uframe_get_cruise_inv_by_array(array_code):
    """ Get cruise inventory list from uframe for an array.
    """
    try:
        base_url, timeout, timeout_read = get_url_info_cruises_inv()
        url = '/'.join([base_url, array_code])
        response = requests.get(url, timeout=(timeout, timeout_read))
        if response.status_code != 200:
            message = 'Unable to get cruise inventory from uframe.'
            raise Exception(message)
...
def get_uframe_event(event_id):
    ...
    try:
        ...
        response = requests.get(url, timeout=(timeout, timeout_read))
        if response.status_code != 200:
            message = 'Unable to get uframe deployment event.'
            raise Exception(message)
        result = response.json()
        event_view = process_deployment_view(result)
        return event_view

    except ConnectionError:
        message = 'ConnectionError getting uframe deployment event.'
        raise Exception(message)
    except Timeout:
        message = 'Timeout getting uframe deployment event.'
        raise Exception(message)
    except Exception as err:
        message = str(err)
        raise Exception(message)
...
def _get_cruise_deployments(cruise_id, type):
    ...
    try:
        # Get deployments
        deployments = get_uframe_deployments_by_cruise_id(cruise_id, type=type)
...
def _get_cruise_deployment(event_id):
    ...
    try:
        # Get deployments
        deployments = get_uframe_event(event_id)
        if not deployments or deployments is None:
            message = 'No deployments for cruise identifier \'%s\'.' % event_id
            raise Exception(message)
        for deploy in deployments:
            result = process_deployment_row(deploy)
            if result is not None:
...
def process_deployment_view(data):
    """ Process deployment information for UI display.
    """
    try:
        print '\n Under construction, get deployment view for UI.'
